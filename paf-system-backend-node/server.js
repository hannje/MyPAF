...
const signatureStorage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, path.join(__dirname, 'data')); // Store in backend/data folder
  },
  filename: function (req, file, cb) {
    // Generate unique filename: timestamp_userId.extension
    const ext = path.extname(file.originalname);
    const userId = req.body.userId || 'unknown';
    const filename = `signature_${Date.now()}_${userId}${ext}`;
    cb(null, filename);
  }
});
...
// GET /api/pafs/custom-ids - Fetch distinct custom IDs for filtering
app.get('/api/pafs/custom-ids', ensureAuthenticated, async (req, res) => {
  let connection;
  try {
    const uspsID = req.session.user.uspsLicenseId;
    console.log("Fetching custom IDs for licensee:", uspsID);

    connection = await dbPool.getConnection();

    const [customIds] = await connection.execute(
      `SELECT DISTINCT CustomID
       FROM pafs
       WHERE licensee_id = ? AND CustomID IS NOT NULL AND CustomID != ''
       ORDER BY CustomID ASC`,
      [uspsID]
    );

    const formattedIds = customIds.map(row => row.CustomID);
    console.log(`Found ${formattedIds.length} distinct custom IDs`);

    res.json(formattedIds);

  } catch (error) {
    console.error('Error fetching custom IDs:', error);
    res.status(500).json({ error: 'Failed to fetch custom IDs.' });
  } finally {
    if (connection) {
      connection.release();
    }
...
// Legacy PAFs endpoint - fetches PAFs from ncoams.paf_cust_info table
app.get('/api/dashboard/legacy-pafs', async (req, res) => {
  console.log('Backend: /api/dashboard/legacy-pafs endpoint hit.', req.session);

  // 1. Check for an active session and a logged-in user
  if (!req.session || !req.session.user || !req.session.user.id) {
    console.log('Backend: /api/dashboard/legacy-pafs - No valid session or user ID found.');
    return res.status(401).json({ message: 'Not authenticated or user session is invalid.' });
  }

  // 2. Ensure the logged-in user is an ADMIN
  if (req.session.user.role !== 'ADMIN') {
    console.log(`Backend: /api/dashboard/legacy-pafs - Access denied. User role is "${req.session.user.role}", not ADMIN.`);
    return res.status(403).json({ message: 'Access denied. Admin role required.' });
  }

  console.log('Backend: /api/dashboard/legacy-pafs - Authenticated admin. Fetching legacy PAFs.');

  let connection;
  try {
    connection = await dbPool.getConnection();

    const [legacyPafs] = await connection.execute(
      'SELECT * FROM ncoams.paf_cust_info ORDER BY list_owner_id DESC'
    );

    console.log(`Backend: /api/dashboard/legacy-pafs - Found ${legacyPafs.length} legacy PAFs`);
    res.json(legacyPafs);

  } catch (error) {
    console.error('Error fetching legacy PAFs for dashboard (/api/dashboard/legacy-pafs):', error);
    res.status(500).json({ message: 'Failed to fetch legacy PAFs.' });
  } finally {
    if (connection) {
      connection.release();
    }
...
app.post('/api/users/upload-signature', uploadSignature.single('signature'), async (req, res) => {
  let connection;

  try {
    console.log('Backend: /api/users/upload-signature endpoint hit.', req.body);

    // Check if user is authenticated
    if (!req.session || !req.session.user || !req.session.user.id) {
      console.log('Backend: /api/users/upload-signature - No valid session or user ID found.');
      return res.status(401).json({ message: 'Not authenticated or user session is invalid.' });
    }

    // Check if file was uploaded
    if (!req.file) {
      console.log('Backend: /api/users/upload-signature - No file uploaded.');
    ...
    if (!targetUserId) {
      console.log('Backend: /api/users/upload-signature - No userId provided.');
      return res.status(400).json({ message: 'User ID is required.' });
    }

    // Only admins can upload signatures for other users, or users can upload for themselves
    const currentUserId = req.session.user.id;
    const isAdmin = req.session.user.role === 'ADMIN';

    if (!isAdmin && currentUserId !== parseInt(targetUserId)) {
      console.log('Backend: /api/users/upload-signature - Access denied. User can only upload their own signature.');
      return res.status(403).json({ message: 'Access denied. You can only upload your own signature.' });
    }

    console.log('Backend: /api/users/upload-signature - File uploaded:', req.file.filename);

    connection = await dbPool.getConnection();

    // Update user record with signature file name
    const [result] = await connection.execute(
      'UPDATE users SET signature_file = ? WHERE id = ?',
      [req.file.filename, targetUserId]
    );

    if (result.affectedRows === 0) {
      console.log('Backend: /api/users/upload-signature - User not found.');
      return res.status(404).json({ message: 'User not found.' });
    }

    console.log(`Backend: /api/users/upload-signature - Signature updated for user ${targetUserId}: ${req.file.filename}`);
    res.json({
    ...
    console.error('Error uploading signature (/api/users/upload-signature):', error);
...
app.get('/api/pafs/my-pafs', ensureAuthenticated, async (req, res) => {
  const loggedInUser = req.session.user;
  const customIdFilter = req.query.customId; // Get custom ID filter from query params
  console.log(`Backend /api/pafs/my-pafs: Called by user ID ${loggedInUser.id}, Role: ${loggedInUser.role}, CustomID Filter: ${customIdFilter || 'None'}`);
  ...
  try {
    ...
    if (loggedInUser.role === 'ADMIN') {
      ...
      let whereClause = 'WHERE p.licensee_id = ?';
      queryParams = [loggedInUser.uspsLicenseId];

      // Add custom ID filter if provided
      if (customIdFilter) {
        whereClause += ' AND p.CustomID = ?';
        queryParams.push(customIdFilter);
      }
      ...
      let whereClause = 'WHERE (p.created_by_user_id = ? or p.agent_id = ?)';
...
app.get('/api/pafs/:pafId/download-pdfxxx', ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (pafData.agent_id) {
        ...
        const agentQuery = 'SELECT licensee_name, first_name, last_name, street_address, city, state, zip, phone_number FROM users WHERE id = ?';
    ...
    drawText(pafData.company, 72, height - 125);
    drawText(pafData.address, 72, height - 150);
    ...
    if (agentData) { // Only draw if agentData was successfully fetched
        ...
        drawText(`${agentData.city || ''}, ${agentData.state || ''} ${agentData.zip || ''}`, 72, height - 570); // Combined city/state/zip
...
app.get('/api/pafs/:pafId/xxxdownload-pdf', ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    drawText(pafData.state, 350, height - 175);
            drawText(`${pafData.zip || ''} - ${pafData.zip4 || ''}`, 450, height - 175);
    ...
    drawText(pafData.parent_company, 72, height - 225);
            drawText(pafData.alt_company, 72, height - 250);
    // ... signature section
    drawText(pafData.signer_name, 72, height - 300);
    drawText(pafData.signer_title, 350, height - 300);
...
// Export PAFs as PALA file format
app.get('/api/export/pafs-pala', authenticateAdmin, async (req, res) => {
  const loggedInAdmin = req.session.user;
  console.log(`Backend /api/export/pafs-pala: Request by admin ID ${loggedInAdmin.id}`);

  if (!loggedInAdmin.uspsLicenseId) {
    return res.status(400).json({ message: "Admin must be associated with a USPS License ID to export PAFs." });
  }
  ...
  try {
    ...
    // Fetch all active PAFs for this licensee
    const query = `
      SELECT
        p.id, p.licensee_id, p.list_owner_sic, p.freq_proc, p.list_owner_id,
        p.company, p.address, p.city, p.state, p.zip, p.zip4, p.telephone,
        p.signer_name, p.signer_title, p.signer_email, p.date_signed,
        p.paf_type, p.list_name, p.mailer_id, p.parent_company, p.alt_company,
        p.agent_id, p.agent_signed_date, 
        p.licensee_sign_date,  p.urbanization, p.list_owner_crid,
        p.CustomID
      FROM pafs p
      WHERE p.licensee_id = ?
      ORDER BY p.id ASC;
    `;
    const [pafs] = await connection.execute(query, [adminUspsLicenseId]);
    ...
    // Generate PALA file content
    let palaContent = '';

    // Header Record (800 chars + CRLF)
    const licenseeId = adminUspsLicenseId.substring(0, 4).padEnd(4, ' ');
    const version = 'V00004';
    const recordCount = padLeft(String(pafs.length), 8);
    const headerFiller = padRight('', 781);
    const headerRecordType = 'H';

    palaContent += licenseeId + version + recordCount + headerFiller + headerRecordType + '\r\n';

    // Detail Records (800 chars + CRLF each)
    for (const paf of pafs) {
      // PAF ID (18 chars): Licensee ID (4) + NAICS (6) + Frequency (2) + List ID (6)
      const licenseePart = adminUspsLicenseId.substring(0, 4).padEnd(4, ' ');
      const naicsPart = (paf.list_owner_sic || '').substring(0, 6).padEnd(6, ' ');
      const freqPart = (paf.freq_proc || '').substring(0, 2).padEnd(2, ' ');
      const listIdPart = (paf.CustomID || paf.list_owner_id || '').substring(0, 6).padEnd(6, ' ');
      const pafId = licenseePart + naicsPart + freqPart + listIdPart; // 18 chars

      const companyName = padRight(paf.company || '', 50);
      const address = padRight(paf.address || '', 50);
      const city = padRight(paf.city || '', 28);
      const state = padRight(paf.state || '', 2);

      // Split ZIP
      const zipParts = (paf.zip || '').split('-');
      const zip5 = padRight(zipParts[0] || '', 5);
      const zip4 = padRight(paf.zip4 || zipParts[1] || '', 4);

      const telephone = padRight((paf.telephone || '').replace(/\D/g, ''), 10);
      const signerName = padRight(paf.signer_name || '', 50);
      const signerTitle = padRight(paf.signer_title || '', 50);
      const dateSigned = formatDate(paf.date_signed);
      const pafType = padRight(paf.paf_type || 'I', 1);
      const listName = padRight(paf.list_name || '', 30);
      const mailerId = padRight(paf.mailer_id || '', 15);
      const filler1 = padRight('', 12);
      const parentCompany = padRight(paf.parent_company || '', 50);
      const altCompany = padRight(paf.alt_company || '', 50);
      const brokerAgentId = padRight(paf.agent_id || '', 16);
      const dateBrokerSigned = formatDate(paf.agent_signed_date);
      const listAdminId = padRight(paf.list_admin_id || '', 16);
      const dateListAdminSigned = formatDate(paf.list_admin_signed_date);
      const dateLicenseeSigned = formatDate(paf.licensee_signature_date);
      const signerEmail = padRight(paf.signer_email || '', 64);
      const website = padRight(paf.website || '', 40);
      const altPafIndicator = padRight('', 1); // Alternative PAF indicator
      const coopDbIndicator = padRight('', 1); // Cooperative DB indicator
      const urbanization = padRight(paf.urbanization || '', 28);
      const uspsCrid = padRight(paf.list_owner_crid || '', 15);
      const filler2 = padRight('', 161);
      const detailRecordType = 'D';

      palaContent += pafId + companyName + address + city + state + zip5 + zip4 +
                     telephone + signerName + signerTitle + dateSigned + pafType +
                     listName + mailerId + filler1 + parentCompany + altCompany +
                     brokerAgentId + dateBrokerSigned + listAdminId + dateListAdminSigned +
                     dateLicenseeSigned + signerEmail + website + altPafIndicator +
                     coopDbIndicator + urbanization + uspsCrid + filler2 + detailRecordType + '\r\n';
    }

    // Set headers to trigger browser download
    const fileName = `${adminUspsLicenseId}.dat`;
    res.setHeader('Content-Type', 'text/plain');
    res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);

    res.status(200).send(palaContent);

  } catch (error) {
    console.error('Error exporting PAFs to PALA:', error);
    res.status(500).json({ message: 'Failed to export PAFs as PALA file.' });
  } finally {
    if (connection) connection.release();
  }
});
...
// Export users as BALA file format
app.get('/api/export/users-bala', authenticateAdmin, async (req, res) => {
  const loggedInAdmin = req.session.user;
  console.log(`Backend /api/export/users-bala: Request by admin ID ${loggedInAdmin.id}`);
  ...
  try {
    ...
    // Query users with broker_list_admin = 'broker' or 'listadmin'
    const query = `
      SELECT
        u.id, u.first_name, u.last_name, u.email, u.role, u.broker_list_admin,
        u.usps_license_id, u.licensee_name, u.street_address, u.city, u.state, u.zip, u.phone_number,
        u.SIC, u.website, u.created_by_admin_id, u.created_at
      FROM users u
      LEFT JOIN users creator_admin ON u.created_by_admin_id = creator_admin.id
      WHERE
        (u.usps_license_id = ? OR creator_admin.usps_license_id = ?)
        AND (u.broker_list_admin = 'broker' OR u.broker_list_admin = 'listadmin')
      ORDER BY u.id ASC;
    `;
    const [users] = await connection.execute(query, [adminUspsLicenseId, adminUspsLicenseId]);

    if (users.length === 0) {
      return res.status(404).json({ message: 'No broker/listadmin users found for your scope to export.' });
    }

    // Helper function to pad string to fixed length
    function padRight(str, length) {
      if (!str) str = '';
      str = String(str).substring(0, length);
      return str.padEnd(length, ' ');
    ...
    // Generate BALA file content
    let balaContent = '';

    // Header Record
    const licenseeId = adminUspsLicenseId.substring(0, 4).padEnd(4, ' ');
    const version = 'V00004'; // NCOALink Report Version Number
    const recordCount = padLeft(String(users.length), 8);
    const headerFiller = padRight('', 281);
    const headerRecordType = 'H';

    balaContent += licenseeId + version + recordCount + headerFiller + headerRecordType + '\r\n';

    // Detail Records
    for (const user of users) {
      // Extract components for PAF ID (16 chars)
      const licenseePart = adminUspsLicenseId.substring(0, 4).padEnd(4, ' ');
      const naicsPart = (user.SIC || '').substring(0, 6).padEnd(6, ' ');
      const balaIdPart = padLeft(String(user.id), 6); // Use user ID as BALA ID
      const pafId = licenseePart + naicsPart + balaIdPart; // Total 16 chars

      const companyName = padRight(user.licensee_name || '', 50);
      const address = padRight(user.street_address || '', 50);
      const city = padRight(user.city || '', 28);
      const state = padRight(user.state || '', 2);

      // Split ZIP into 5-digit and 4-digit parts
      const zipParts = (user.zip || '').split('-');
      const zip5 = padRight(zipParts[0] || '', 5);
      const zip4 = padRight(zipParts[1] || '', 4);

      const telephone = padRight((user.phone_number || '').replace(/\D/g, ''), 10);
      const filler1 = padRight('', 12);

      // BALA Indicator: B for broker, L for listadmin
      let balaIndicator = ' ';
      if (user.broker_list_admin === 'broker') {
        balaIndicator = 'B';
      } else if (user.broker_list_admin === 'listadmin') {
        balaIndicator = 'L';
      }

      const website = padRight(user.website || '', 40);
      const urbanization = padRight('', 28); // Not stored in users table
      const filler2 = padRight('', 53);
      const detailRecordType = 'D';

      balaContent += pafId + companyName + address + city + state + zip5 + zip4 +
                     telephone + filler1 + balaIndicator + website + urbanization +
                     filler2 + detailRecordType + '\r\n';
    }
    ...
    res.status(200).send(balaContent);

  } catch (error) {
    console.error('Error exporting users to BALA:', error);
    res.status(500).json({ message: 'Failed to export users as BALA file.' });
  } finally {
    if (connection) connection.release();
  }
});
...
// --- NEW ENDPOINT FOR EXPORTING/UPSERTING A PAF ---
app.post('/api/pafs/:pafId/export-to-ncoamszzz', authenticateAdmin, async (req, res) => {
  const pafIdToExport = parseInt(req.params.pafId, 10);
  const loggedInAdmin = req.session.user;
  console.log(`Backend /export-to-ncoams: Request for PAF ID ${pafIdToExport} by Admin ID ${loggedInAdmin.id}`);
  ...
  try {
    ...
    // 1. Fetch all source data from your current database (paf_management_db)
    const sourceQuery = `
      SELECT 
        p.*,
        u_licensee.usps_license_id AS licensee_platform_id,
    ...
    console.log(sourcePaf);

    
    // As requested: Log the agent info if present
    if (sourcePaf.agent_id) {
        console.log(`Exporting PAF with Agent ID: ${sourcePaf.agent_id}, Agent SIC: ${sourcePaf.agent_sic}`);
    }
    ...
    if (upsertResult.affectedRows === 2) { // In MySQL, an update counts as 2 affected rows with ON DUPLICATE KEY UPDATE
        actionTaken = 'updated';
    }
    console.log(`Backend /export-to-ncoams: Record for list_owner_id ${sourcePaf.list_owner_id} was ${actionTaken}.`);


    const historyNotes = `PAF data exported to NCOAMS by admin ${loggedInAdmin.email}. Action: ${actionTaken}.`;
    // We use the PAF's *current* status for the history log, as the export doesn't change it.
    const historyQuery = `
      INSERT INTO paf_status_history (paf_id, status, notes, changed_by_user_id, changed_at) 
      VALUES (?, ?, ?, ?, NOW());
    `;
    await connection.execute(historyQuery, [
      pafIdToExport,
      sourcePaf.status, // Use the current status of the PAF
      historyNotes,
      loggedInAdmin.id
    ]);
    ...
    console.log(`Backend /export-to-ncoams: Record for list_owner_id ${sourcePaf.list_owner_id} was ${actionTaken} and history was logged.`);



    res.status(200).json({ message: `PAF data successfully ${actionTaken} in the NCOAMS database.` });
